...
public class MemberListController implements Initializable {
    ...
    private void handleMemberDelete(ActionEvent event) {
        ...
        if (selectedForDeletion == null) {
            AlertMaker.showErrorMessage("Uye secmediniz", "Lutfen silmek icin uye seciniz.");
            return;
        }
        if(DatabaseHandler.getInstance().isMemberHasAnyBooks(selectedForDeletion))
        {
            AlertMaker.showErrorMessage("Silnemez", "Bu uyede emanet kitab vardir.");
            return;
        }
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Kitab Silme");
        alert.setContentText("Emin misiniz bu " + selectedForDeletion.getName()+ " kitabi silmeye?");
        Optional<ButtonType> answer = alert.showAndWait();
        if (answer.get() == ButtonType.OK) {
            Boolean result = DatabaseHandler.getInstance().deleteMember(selectedForDeletion);
            if (result) {
                AlertMaker.showSimpleAlert("Kitab silindi ", selectedForDeletion.getName()+ " silindi.");
                list.remove(selectedForDeletion);
            } else {
                AlertMaker.showSimpleAlert("Basarisiz", selectedForDeletion.getName()+ " silinemiyor");
            }
        } else {
            AlertMaker.showSimpleAlert("Silme iptal oldu", "Islme Iptal");
        }
    }
    ...
    private void handleMemberEdit(ActionEvent event) 
    {
        ...
        if (selectedForEdit == null) {
            AlertMaker.showErrorMessage("Uye secmediniz", "Lutfen guncellemek icin uye seciniz.");
            return;
        }
        try {
